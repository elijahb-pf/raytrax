...
def inverse_fourier_transform(
    fourier_coefficients: jt.Float[jax.Array, "n_surfaces n_fourier_coefficients"],
    ...
    s_theta_phi: jt.Float[jax.Array, "n_s n_theta n_phi 3"],
    basis: FourierBasis,
) -> jt.Float[jax.Array, "n_s n_theta n_phi"]:
    """Transform Fourier coefficients into a spatial representation."""
    angle = (
        poloidal_mode_numbers[:, jnp.newaxis, jnp.newaxis, jnp.newaxis]
        * s_theta_phi[jnp.newaxis, :, :, :, 1]
        - toroidal_mode_numbers[:, jnp.newaxis, jnp.newaxis, jnp.newaxis]
        * s_theta_phi[jnp.newaxis, :, :, :, 2]
    )
